...
해당 경우에는 [채널 접속 종료 이벤트](/sdk-docs/backend/chat/channel-guild/exit/event)가 발생하며, 다시 [채널에 접속을 요청](/sdk-docs/backend/chat/channel-guild/connect/request)해야 합니다.  

> 나가진 채널에 다시 접속을 하기 위해 JoinChannel을 호출할 경우, 방의 인원이 꽉 차있다면 채널에 접속이 되지 않을 수 있습니다.  

## SDK 5.4.5 이하 버전

뒤끝챗에서는 [클라이언트와 채팅 서버가 연결이 끊어지는 경우](/sdk-docs/backend/chat/channel-guild/exit/event) 자동으로 재접속을 시도합니다.  
...
**길드 채널 재접속 성공 시**  
[길드 채널 접속 이벤트](/sdk-docs/backend/chat/channel-guild/connect/event)가 호출됩니다.  

**길드 채널 재접속 실패 시**  
[길드 채널 접속 종료 이벤트](/sdk-docs/backend/chat/channel-guild/exit/event)가 호출됩니다.  

## 다른 유저가 재접속한 경우

**길드 채널 재접속 성공 시**  
[재접속 성공 이벤트](/sdk-docs/backend/chat/channel-guild/reconnect/old-reconnect)가 호출됩니다.  

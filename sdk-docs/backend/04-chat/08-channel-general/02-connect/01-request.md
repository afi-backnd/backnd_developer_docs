...
- 실제 서버 연결은 채팅 서버에서 유저 인증을 완료한 후 이루어지며, **유저의 일반 채팅 채널 접속 성공/실패 여부는 [OnJoinChannel](/sdk-docs/backend/chat/channel-general/connect/event) 이벤트를 통해 확인**할 수 있습니다.  
...
```js
JsonData channel = Backend.Chat.GetGroupChannelList("groupName").Rows()[0];
...
이 상태는 단순히 소켓 연결에 성공한 것으로 아직 채팅 서버에 접속된 것이 아닙니다.  
true  

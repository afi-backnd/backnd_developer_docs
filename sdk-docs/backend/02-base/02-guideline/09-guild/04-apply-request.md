...
void Test()
{
    ...
    BackendGuild.Instance.RequestGuildJoin("원하는_길드_이름"); // Step 2에서 CreateGuild에 입력한 길드이름을 인자값으로 입력해주세요
...
```js
    void Test() {
        ...
        BackendLogin.Instance.CustomLogin("user1", "1234"); // Step 2에서 길드를 생성한 유저로 로그인해야합니다.  

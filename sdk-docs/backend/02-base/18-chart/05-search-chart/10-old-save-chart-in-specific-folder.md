...
draft: "true"
unlisted: "true"
description: "[Deprecated] GetChartByFolderAndSave"
---

# [Deprecated] GetChartByFolderAndSave
public BackendReturnObject **GetChartByFolderAndSave**(int folderId, bool isChartKeyIsName);    
...
### 동기
```js
Backend.Chart.GetChartByFolderAndSave(1024, true);
```

### 비동기
```js
Backend.Chart.GetChartByFolderAndSave(1024, true, callback =>
{
    // 이후 처리
});
```
...
### SendQueue
```js
SendQueue.Enqueue(Backend.Chart.GetChartByFolderAndSave, 1024,  true, callback =>
{
    // 이후 처리
});
```

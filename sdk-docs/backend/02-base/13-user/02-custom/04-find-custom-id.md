...
### 동기
```js
Backend.BMember.FindCustomID("help@backnd.com");
```

### 비동기
```js
Backend.BMember.FindCustomID("help@backnd.com", (callback) =>
{
    // 이후 처리
});
```
...
### SendQueue
```js
SendQueue.Enqueue(Backend.BMember.FindCustomID, "help@backnd.com", (callback) =>
{
    // 이후 처리
});
```

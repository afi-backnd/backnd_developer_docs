...
# FindAccountId
public BackendReturnObject **FindAccountId**(string **EmailAddress**);
...
## 설명
유저가 멀티 캐릭터 계정 아이디를 잊어버렸거나 비밀번호를 초기화하고 싶을 때, 미리 등록해둔 유저의 이메일을 통해 아이디를 찾고, 비밀번호를 초기화할 수 있습니다.  
...
### 동기
```js
Backend.MultiCharacter.Account.FindAccountId("help@backnd.com");
```

### 비동기
```js
Backend.MultiCharacter.Account.FindAccountId("help@backnd.com", (callback) =>
{
    // 이후 처리
});
```
...
### SendQueue
```js
SendQueue.Enqueue(Backend.MultiCharacter.Account.FindAccountId, "help@backnd.com", (callback) =>
{
    // 이후 처리
});
```

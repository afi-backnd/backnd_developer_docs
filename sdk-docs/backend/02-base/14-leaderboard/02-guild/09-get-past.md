...
초기화 날짜가 지정된 1회성 리더보드의 경우 지정한 초기화 날짜 이전에 `GetPastLeaderboard` 또는 `GetPastRankList`를 호출하여 전체 순위 데이터를 요청하게 되면 에러값을 리턴합니다.  
...
Backend.Leaderboard.Guild.GetPastLeaderboard("a61c6980-db5f-11ee-95a9-73cbb1225013", 50, 10);
```

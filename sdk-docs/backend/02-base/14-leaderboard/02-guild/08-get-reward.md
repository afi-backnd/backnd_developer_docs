...
### 동기
```js
BackEnd.Leaderboard.BackendLeaderboardRewardReturnObject bro = Backend.Leaderboard.Guild.GetLeaderboardReward("uuid");
...
### 비동기
```js
Backend.Leaderboard.Guild.GetLeaderboardReward("leaderboardUuid", bro => {
    foreach(BackEnd.Leaderboard.LeaderboardRewardItem item in bro.GetLeaderboardRewardList())
    {
        Debug.Log(item);
        if(item.rewardItems.ContainsKey("level"))

...
:::danger 안내
리더보드에 등록되어있는 테이블을 **Public** 으로 변경할 경우, **400 bad public Table** 에러가 발생하며 갱신되지 않습니다.  
등록된 테이블을 **Public** 으로 변경하는 것은 삼가해주시기 바랍니다.  
:::

:::warning 데이터베이스 연동 주의사항
데이터베이스를 사용하여 리더보드와 연동할 경우, 반드시 SDK 버전이 **5.18.5** 이상인지 확인해주시기 바랍니다.  
...
* UpdateMyDataAndRefreshLeaderboard 함수는 [Backend.GameData.Update](/sdk-docs/backend/base/game-information/update/using-indate) 함수에 리더보드 갱신 기능이 추가된 함수 입니다. 
...
**1. 게임 정보 사용**
- `Backend.GameData.Get()`, `Backend.GameData.Insert()` 등을 사용하여 데이터를 관리합니다.
- Param을 통해 데이터를 추가/수정하며, rowIndate를 직접 관리해야 합니다.
- 자세한 내용은 [게임 정보 문서](/sdk-docs/backend/base/game-information/sample)를 참고하세요.

**2. 데이터베이스 사용**
- C# 클래스로 테이블 구조를 정의하여 컴파일 시점에 타입 체크를 수행합니다.
- `GetPrimaryKey()` 메서드를 통해 rowIndate를 자동으로 추출할 수 있어 편리합니다.
- 자세한 내용은 [데이터베이스 문서](/sdk-docs/database/intro)를 참고하세요.
...
string target = "갱신할 row inDate"; 
// 데이터베이스 사용시 
// target = row.GetPrimaryKey();

Backend.Leaderboard.User.UpdateMyDataAndRefreshLeaderboard("리더보드 uuid", "테이블 이름", target, param);
```
...
Backend.Leaderboard.User.UpdateMyDataAndRefreshLeaderboard("리더보드 uuid", "테이블 이름", target, param, callback => 
{
    // 이후 처리
});
```

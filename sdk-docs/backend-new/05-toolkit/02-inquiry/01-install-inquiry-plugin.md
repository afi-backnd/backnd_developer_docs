...
Android : [BackendQuestion-Android-1.0.0.unitypackage](https://developer.thebackend.io/sdk/question/BackendQuestion-Android-1.0.0.unitypackage) \[2024-03-26]  
...
```js
// Task 함수
var reqResult = await BackndInquiry.Instance.GetInquiryAuthCodeAsync();
string questionAuthorize = reqResult.GetAuthCode():

// Callback 함수
BackndInquiry.Instance.GetInquiryAuthCode(callback =>
{
    string questionAuthorize = reqResult.GetAuthCode():
});
```
...
#if UNITY_ANDROID
TheBackend.ToolKit.Question.Android.OpenQuestionView(questionAuthorize, BackndBase.UserInDate, (error) =>
{
    Debug.LogError("1:1 문의창 활성화중 에러가 발생했습니다 : " + error);
});

#elif UNITY_IOS
TheBackend.ToolKit.Question.iOS.OpenQuestionView(questionAuthorize, BackndBase.UserInDate, (error) =>
{
    Debug.LogError("1:1 문의창 활성화중 에러가 발생했습니다 : " + error);
});
#endif
...
public class NewBehaviourScript : MonoBehaviour
{
    ...
    async void Start()
    {
        var initResult = await BackndBase.InitializeAsync();
        ...
        {
            var bro2 = await BackndInquiry.Instance.GetInquiryAuthCodeAsync();
            authCode = bro2.GetAuthCode();
        }
    }

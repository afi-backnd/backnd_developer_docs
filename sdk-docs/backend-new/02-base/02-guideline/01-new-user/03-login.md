...
## 1. 로그인 스크립트 작성
사전 준비에서 작성한 [BackendLogin.cs](/sdk-docs/backend/base/guideline/new-user/before)의 CustomSignIn 함수에 내용을 추가합니다.  
...
async void Test()
{
    await BackendLogin.Instance.CustomSignIn("user1", "1234"); // [추가] 뒤끝 회원가입 함수
    Debug.Log("테스트를 종료합니다.");
}
```

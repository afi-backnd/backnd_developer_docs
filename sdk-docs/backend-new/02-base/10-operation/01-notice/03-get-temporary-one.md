...
public Task&lt;GetTempAnnouncementResult&gt; **GetTempAnnouncementAsync**();
...
### Task 형식
```js
var reqResult = await BackndAnnouncement.Instance.GetTempAnnouncementAsync();
Debug.Log(reqResult);
```

### Callback 형식
```js
BackndAnnouncement.Instance.GetTempAnnouncement((callback) =>
{
    // 이후 처리
    Debug.Log(callback);
});

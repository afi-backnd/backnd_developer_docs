...
description: "이벤트 리스트 조회"
sidebar_position: 1
---

# GetEvents

public Task&lt;GetEventsResult&gt; **GetEventsAsync**();  
public Task&lt;GetEventsResult&gt; **GetEventsAsync**(int **limit**);  
public Task&lt;GetEventsResult&gt; **GetEventsAsync**(int **limit**, string **offset**);
...
| offset | string | (Optional) 불러올 이벤트의 시작점  (GetEventsResult.GetOffsetKey()를 통해 얻으실 수 있습니다.) |
...
{
    //10개를 불러온 이후에 이벤트 10
    var offset = reqResult.GetOffsetKey();
    if (!string.IsNullOrEmpty(offset))
    {
        // 2. 불러온 이벤트 2개 이후의 3개 불러오기
        var reqResult2 = await BackndEvent.Instance.GetEventsAsync(3, offset);
    }
}
```
...
BackndEvent.Instance.GetEvents(2, (callback) =>
{
    ...
    {
        var offset = callback.GetOffsetKey();
        if (!string.IsNullOrEmpty(offset))
        {
            // 2. 불러온 이벤트 2개 이후의 3개 불러오기
            BackndEvent.Instance.GetEvents(3, offset, (callback2) =>
            {
                // 이후 처리
            });
        }

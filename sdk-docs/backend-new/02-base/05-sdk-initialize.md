...
# Initialize
public Task< RequestResult > **InitializeAsync**();  
public Task< RequestResult > **InitializeAsync**(BackendCustomSetting **settings**);  
public void **Initialize**(BackendCallback **callback**);  
public void **Initialize**(BackendCustomSetting **settings**, BackendCallback **callback**);
...
| callback  | Action&lt;RequestResult&gt; | 초기화 성공/실패 시 호출할 비동기 함수의 콜백 함수 | 
...
{
    // 첫 번째 방법(동기)
    var bro = await BackndBase.InitializeAsync();
    if(bro.IsSuccess())
    {
        // 초기화 성공 시 로직
    }
...
async void Start()
{
    ...
    var bro = await BackndBase.InitializeAsync(settings);
    if(bro.IsSuccess())
    {
        // 초기화 성공 시 로직
    }
...
{
    // 세 번째 방법(비동기)
    BackndBase.Initialize(callback => {
...
void Start()
{
    ...
    BackndBase.Initialize(settings, callback => {
        if(callback.IsSuccess())
        {
            // 초기화 성공 시 로직
        }

...
새로운 우편 기능(**BackndMail**)의 불러오기 기능은 자신이 받을 수 있는 제일 오래된 우편부터 limit의 수만큼 불러오기 시작합니다.  
...
> var reqRequest = await BackndMail.Instance.GetMailsAsync(MailType.Admin, 100);
...
public async Task GetPostList()
{
    ...
    for (int i = 0; i < infoList.Count; i++)
    {
        ...
        string postTitle = info.Title; // 우편 제목
        string postContent = info.Content; // 우편 내용
        string postAuthor = info.Author; // 우편 저자
        int postItemCount = info.ItemList.Count; // 문제가 되는 데이터타입 제거로 인해 코드 간편화
        string postIndate = info.InDate;

        await ReceivePostItem(MailType.Admin, postIndate); // 우편 수령
    }
}
public async Task ReceivePostItem(MailType mailType, string postIndate)
{
    var reqResult = await BackndMail.Instance.ReceiveMailAsync(mailType, postIndate);
    var postList = reqResult.AsSystemResult().GetInfoList();
    for (int i = 0; i < postList.Count; i++)
    {
        ...
        string itemName = info.ItemFields["itemName"];

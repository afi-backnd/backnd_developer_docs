...
# UpdateNickname
public Task&lt;RequestResult&gt; **UpdateNicknameAsync**(string **nickname**);
...
### Task 방식
```js
var reqResult = await BackndAuth.Instance.UpdateNicknameAsync("thebackend");        
```

### Callback 방식
```js
BackndAuth.Instance.UpdateNickname("thebackend", (callback) =>
{
    // 이후 처리
});
```

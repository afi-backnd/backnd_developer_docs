...
# DeleteAccount
public Task&lt;RequestResult&gt; **DeleteAccountAsync**();  
public Task&lt;RequestResult&gt; **DeleteAccountAsync**(int graceHours);
...
유예 시간이 있을 경우 로그인 시 탈퇴 철회.</td>
  ...
  <tr>
    <td>데이터 삭제</td>    
    <td>푸시, 길드, 친구, 랭킹, 닉네임, 페더레이션, 국가 코드 + 내 모든 게임 정보 관리 데이터</td>
  </tr>
</table>
...
```js
//즉시 탈퇴
var reqResult = await BackndAuth.Instance.DeleteAccountAsync();

//2시간 뒤에 탈퇴 예약
var reqResult = await BackndAuth.Instance.DeleteAccountAsync(2);

//7일 뒤에 탈퇴 예약
var reqResult = await BackndAuth.Instance.DeleteAccountAsync(24 * 7);
```

### Callback 방식
```js
//즉시 탈퇴
BackndAuth.Instance.DeleteAccount(callback  =>
{
    // 이후 처리
});

//2시간 뒤에 탈퇴 예약
BackndAuth.Instance.DeleteAccount(2, callback =>
{
    // 이후 처리
});
```

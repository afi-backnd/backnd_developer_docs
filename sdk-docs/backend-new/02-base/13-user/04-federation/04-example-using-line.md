...
public void StartLineFederation()
{
    ...
    {
        result.Match(async loginResult =>
        {
            ...
            var reqResult = await BackndAuth.Instance.SignInWithProviderAsync
                (loginResult.AccessToken.IdTokenRaw, AuthProvider.Line);                
            if (reqResult.IsSuccess())
            {
                Debug.Log("라인 연동 로그인 성공 : " + reqResult);
                await BackndAuth.Instance.UpdateProviderEmailAsync(loginResult.UserProfile.UserId, AuthProvider.Line);
                BackendInit.instance.SetFederationName("라인 로그인");
            }
            else
            {
                Debug.Log("라인 연동 로그인 실패 : " + reqResult);
            }
        },
        error =>
        {

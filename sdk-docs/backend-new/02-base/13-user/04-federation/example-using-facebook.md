...
5. 페이스북 액세스 토큰을 이용해 BackndAuth.Instance.SignInWithProviderAsync("accessToken", FederationType.FaceBook) 호출
...
페이스북 로그인 연동을 위해서는 해당 public_profile 접근 권한과 email 접근 권한이 필요합니다.  
...
| --- | --- | --- |
| 개발 모드 | 액세스레벨 관계없이 뒤끝 외부 로그인 인증이 가능합니다. | 고급 액세스 레벨 권한이 없으면 뒤끝 외부 로그인 인증은 불가능합니다. |
| 라이브 모드 | 고급 액세스 레벨 권한이 없으면 뒤끝 외부 로그인 인증이 불가능합니다. | 고급 액세스 레벨 권한이 없으면 뒤끝 외부 로그인 인증이 불가능합니다. |
> 모드 구분 없이 페이스북 SDK를 통해 개발 계정, 일반 계정, 테스트 계정 모두 페이스북 계정 로그인은 가능하나, 
**고급 액세스 레벨 권한이 없으면 뒤끝 서버에서 페이스북 서버와 인증하는 과정에서 테스트 및 개발 계정을 제외하고는 인증이 실패하기 때문에 외부 로그인 인증이 불가능**합니다.
...
private async void AuthCallback ( ILoginResult result )
{
    ...
    if (FB.IsLoggedIn) {
        ...
        await BackndAuth.Instance.SignInWithProviderAsync(facebookToken, AuthProvider.Facebook);

...
| provider        | AuthProvider   | 외부 로그인 제공자의 종류.(AuthProvider.Steam)                    |
...
```js
    ...
    async void Start()
    {
        await BackndBase.InitializeAsync();
...
```js
var reqResult = await BackndAuth.Instance.SignInWithProviderAsync(sessionTicket, AuthProvider.Steam);

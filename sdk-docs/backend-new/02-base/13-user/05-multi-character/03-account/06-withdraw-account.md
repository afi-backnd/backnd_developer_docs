...
# WithdrawAccount
public Task&lt;RequestResult&gt; **WithdrawAccountAsync**();  
public Task&lt;RequestResult&gt; **WithdrawAccountAsync**(int expirationHours);
...
```js
//즉시 탈퇴
var reqResult = await BackndMultiCharacter.Account.WithdrawAccountAsync();

//2시간 뒤에 탈퇴 예약
var reqResult = await BackndMultiCharacter.Account.WithdrawAccountAsync(2);
```

### Callback 방식
```js
//즉시 탈퇴
BackndMultiCharacter.Account.WithdrawAccount(callback  =>
{
    // 이후 처리
});

//2시간 뒤에 탈퇴 예약
BackndMultiCharacter.Account.WithdrawAccount(2, callback =>
{
    // 이후 처리
});
```

...
public Task&lt;RequestResult&gt; **ConfirmCustomPasswordAsync**(string **CustomGamerPassword**);
...
### Task 방식
```js
var reqResult = await BackndMultiCharacter.Account.ConfirmCustomPasswordAsync("1234");
...
### Callback 방식
```js
BackndMultiCharacter.Account.ConfirmCustomPassword("1234", (callback) =>
{
...
# ResetPassword
public Task&lt;RequestResult&gt; **ResetPasswordAsync**(string **CustomGamerId**, string **EmailAddress**);
...
## 설명
멀티 캐릭터 계정의 커스텀 아이디와 등록한 이메일 정보를 입력 시, 해당 이메일로 초기화된 비밀번호를 전송합니다.  
...
### Task 방식
```js
var reqResult = await BackndMultiCharacter.Account.ResetPasswordAsync("CustomGamerId" , "help@backnd.com");
```

### Callback 방식
```js
BackndMultiCharacter.Account.ResetPassword("CustomGamerId" , "help@backnd.com", (callback) =>
{
  // 이후 처리
});
```
...
# UpdatePassword
public Task&lt;RequestResult&gt; **UpdatePasswordAsync**(string **OldPassword**, string **NewPassword**);
...
### Task 방식
```js
var reqResult = await BackndMultiCharacter.Account.UpdatePasswordAsync("OldPassword", "NewPassword");
```

### Callback 방식
```js
BackndMultiCharacter.Account.UpdatePassword("OldPassword", "NewPassword", (callback) =>
{
    // 이후 처리
});
```

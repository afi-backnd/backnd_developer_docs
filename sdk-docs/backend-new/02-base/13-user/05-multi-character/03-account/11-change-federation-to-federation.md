...
description: "[New] 페더레이션 계정으로 전환"
---

# ChangeToFederation
public Task&lt;RequestResult&gt; **ChangeToFederationAsync**(string **federationToken**, FederationType **type**);  
...
### Task 방식
```js
var reqResult = await BackndMultiCharacter.Account.ChangeToFederationAsync("federationToken" , FederationType.Google);
```

### Callback 방식
```js
BackndMultiCharacter.Account.ChangeToFederation("federationToken" , FederationType.Google, callback =>
{
    // 페더레이션 인증 이후 처리
});
```

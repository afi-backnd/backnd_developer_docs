...
public Task&lt;RequestResult&gt; **ChangeCustomToFederationAsync**(string **federationToken**, FederationType **type**);  
...
### Task 방식
```js
var reqResult = await BackndMultiCharacter.Account.ChangeCustomToFederationAsync("federationToken" , FederationType.Google);
```

### Callback 방식
```js
BackndMultiCharacter.Account.ChangeCustomToFederation("federationToken" , FederationType.Google, callback =>
{
    // 페더레이션 인증 이후 처리
});
```

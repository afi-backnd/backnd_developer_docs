...
| Value   | Description  |
| :-------- | :-----|
| BackndBase.UserNickName   | 유저의 닉네임 |
| BackndBase.UserInDate   | 유저의 inDate |
| BackndBase.UID   | 유저의 웹 쿠폰용 UID |

```js
var reqResult = await BackndAuth.Instance.SignInCustomAsync("backendUser", "backendUser");
if (reqResult.IsSuccess())
{
    Debug.Log("로그인에 성공했습니다 : " + reqResult);
    Debug.Log($"유저 닉네임 : " + BackndBase.UserNickName);
    Debug.Log($"유저 인데이트 : " + BackndBase.UserInDate);
    Debug.Log($"유저 UID(쿠폰용) : " + BackndBase.UID);
}
else
{
    Debug.LogError("로그인 중 에러가 발생했습니다.");
    // BackndBase.UserNickname, BackndBase.UserInDate, BackndBase.UID에 값이 할당되지 않습니다.  
}
```

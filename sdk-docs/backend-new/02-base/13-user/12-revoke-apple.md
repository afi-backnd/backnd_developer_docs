...
description: "애플 토큰 만료(only iOS)"
---

# RevokeAppleToken
public Task&lt;RequestResult&gt; **RevokeAppleTokenAsync**(string **authCode**);

## 파라미터

| Value        | Type           | Description  |
| :------------ |:------------| :-----|
| authCode | string  | 애플 서버에 전송할 애플의 유저 인증 값 |
    ...
    this.appleAuthManager.LoginWithAppleId(
        ...
        async credential =>
        {
            ...
            if (appleIdCredential != null)
            {
                ...
                var reqResult = await BackndAuth.Instance.RevokeAppleTokenAsync(authorizationCode);
                Debug.Log("RevokeAppleToken : " + reqResult);

                if (reqResult.IsSuccess())
                {
                    var reqResult2 = await BackndAuth.Instance.DeleteAccountAsync();

                    Debug.Log("회원탈퇴 : " + reqResult);
                }
            }
        },
        error =>
...
BackndAuth.Instance.RevokeAppleToken(authorizationCode, callback  => 
{
    if(callback.IsSuccess()) 
    {
      BackndAuth.Instance.DeleteAccount(callback2 => 
      {

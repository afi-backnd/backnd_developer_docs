...
public Task&lt;RequestResult&gt; **RefreshBackndTokenAsync**();
...
### Task 방식
```js
var reqResult = await BackndAuth.Instance.RefreshBackndTokenAsync();
```

### Callback 방식
```js
BackndAuth.Instance.RefreshBackndToken((callback) =>
{
    // 이후 처리
});
```

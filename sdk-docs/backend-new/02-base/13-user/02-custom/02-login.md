...
public Task&lt;RequestResult&gt; **SignInCustomAsync**(string **id**, string **password**);  
public Task&lt;RequestResult&gt; **SignInCustomAsync**(string **id**, string **password**, string **etc**);
...
### Task 방식
```js
var reqResult = await BackndAuth.Instance.SignInCustomAsync("id", "password");
...
### Callback 방식
```js
BackndAuth.Instance.SignInCustomAsync("id", "password", reqResult =>
{
    if (reqResult.IsSuccess())
    {
        Debug.Log("로그인에 성공했습니다");

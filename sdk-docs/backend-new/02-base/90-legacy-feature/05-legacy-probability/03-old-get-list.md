...
description: "[Deprecated]  뽑기 리스트 조회"
---
...
```js
var reqResult = await BackndLegacy.Probability.GetProbabilityCardListAsync();
```

### Callback 방식

```js
BackndLegacy.Probability.GetProbabilityCardList((callback) =>
{
  // 이후 처리
});
```
...
```js
public async Task GetProbabilityCardListTest()
{
    var reqResult = await BackndLegacy.Probability.GetProbabilityCardListAsync();
    ...
    var probabilityCardList = new List<ProbabilityCard>();

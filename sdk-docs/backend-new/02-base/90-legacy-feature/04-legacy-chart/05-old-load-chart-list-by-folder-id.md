...
description: "[Deprecated] 폴더 ID를 통한 차트 리스트 불러오기"
---

# [Deprecated] GetChartListByFolder
public Task< RequestResult > **GetChartListByFolderAsync**(int folderId);
...
```js
int folderId = 000;
var reqResult = await BackndLegacy.Chart.GetChartListByFolderAsync(folderId);
```
...
BackndLegacy.Chart.GetChartListByFolder(folderId, (callback) => {
    // 이후 작업
});
```
...
public async Task GetChartListByFolder()
{
    int folderId = 343;
    ...
    var chartCardList = new List<ChartCard>();

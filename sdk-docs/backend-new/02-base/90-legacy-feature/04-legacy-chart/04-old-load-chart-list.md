...
public Task< RequestResult > **GetChartListAsync**();
...
### Task 방식
```js
var reqResult = await BackndLegacy.Chart.GetChartListAsync();
```

### Callback 방식
```js
BackndLegacy.Chart.GetChartList((callback) => 
{
    // 이후 작업
});
```
...
public async Task GetChartListTest()
{
    var reqResult = await BackndLegacy.Chart.GetChartListAsync();

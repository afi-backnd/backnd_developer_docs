...
public Task< RequestResult > **GetChartListByFolderV2Async**(int **folderId**);
...
:::caution GetChartListByFolder 마이그레이션 안내
...
```js
int folderId = 0;
var reqResult = await BackndLegacy.Chart.GetChartListByFolderV2Async(folderId);
```
...
BackndLegacy.Chart.GetChartListByFolderV2(folderId, (callback) => {
    // 이후 작업
});
```
...
```js
public async Task GetChartListByFolderV2Test()
{
    int folderId = 343;

...
public Task< RequestResult > **GetChartByFolderAndSaveV2Async**(int **folderId**, bool **isChartKeyIsName**);
...
### GetChartByFolderAndSave와의 차이점
GetChartByFolderAndSave()와 기능은 거의 동일하지만 다음과 같은 차이점이 존재합니다.  
* 파일이 적용되지 않는 차트는 리스트에서 제외
* Json 리턴값중 old 컬럼 제거

:::caution GetChartByFolderAndSave 마이그레이션 안내
GetChartByFolderAndSave() 함수에서 GetChartByFolderAndSaveV2() 함수로 마이그레이션 시 다음과 같은 사항이 있는지 확인해주세요.  
...
### Task 방식
```js
var reqResult = await BackndLegacy.Chart.GetChartByFolderAndSaveV2Async(1024, true);
```

### Callback 방식
```js
BackndLegacy.Chart.GetChartByFolderAndSaveV2(1024, true, callback =>
{
    // 이후 처리
});
```
...
```js
public async Task GetChartByFolderAndSaveV2Test()
{
    int folderId = 343;

...
public Task< RequestResult > **GetOneChartAndSaveAsync**(string **chartFileId**);  
public Task< RequestResult > **GetOneChartAndSaveAsync**(string **chartFileId**, string **chartName**);
...
### Task 방식
```js
var reqResult = await BackndLegacy.Chart.GetOneChartAndSaveAsync("chartFileId");
var reqResult2 = await BackndLegacy.Chart.GetOneChartAndSaveAsync("chartFileId", "차트 이름");
```

### Callback 방식
```js
BackndLegacy.Chart.GetOneChartAndSave("uuid", callback =>
{
    // 이후 처리
});
BackndLegacy.Chart.GetOneChartAndSave("uuid", "차트 이름", callback =>
{
    // 이후 처리
});
```
...
public async Task GetOneChartAndSaveTest()
{
    string selectedProbabilityFileId = "560";

    var reqResult = await BackndLegacy.Chart.GetOneChartAndSaveAsync(selectedProbabilityFileId);

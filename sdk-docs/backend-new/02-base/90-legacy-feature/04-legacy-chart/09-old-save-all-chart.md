...
public Task< RequestResult > **GetAllChartAndSaveAsync**(bool **isChartKeyIsName**);
...
### Task 방식
```js
var reqResult = await BackndLegacy.Chart.GetAllChartAndSaveAsync(true);
```

### Callback 방식
```js
BackndLegacy.Chart.GetAllChartAndSave(true, callback =>
{
    // 이후 처리
});
```

...
# GetMyRank
public Task&lt;RequestResult&gt; **GetMyRankAsync**(string **rankUuid**);  
public Task&lt;RequestResult&gt; **GetMyRankAsync**(string **rankUuid**, int **gap**);
...
```js
// 자기 자신의 랭킹만 조회
var reqRequest = await BackndLegacy.URank.User.GetMyRankAsync("rankUuid");

// 위아래 3명의 유저를 포함하여 랭킹 조회
// 자신이 4등인 경우 1 ~ 7등 유저가 조회됨
var reqRequest2 = await BackndLegacy.URank.User.GetMyRankAsync("rankUuid", 3);
```

### Callback 방식
```js
// 자기 자신의 랭킹만 조회
BackndLegacy.URank.User.GetMyRank("rankUuid", callback =>
{
    // 이후 처리
});
...
BackndLegacy.URank.User.GetMyRank("rankUuid", 5, callback =>
{
    // 이후 처리
});
```
...
public async Task GetMyRankTest()
{
    ...
    var reqResult = await BackndLegacy.URank.User.GetMyRankAsync(userUuid, limit);

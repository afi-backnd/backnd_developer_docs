...
public Task&lt;RequestResult&gt; **GetRankTableListAsync**();  
...
### Task 방식
```js
var reqResult = await BackndLegacy.URank.User.GetRankTableListAsync();
```

### Callback 방식
```js
BackndLegacy.URank.User.GetRankTableList(callback=>
{
    // 이후 처리
});
```
...
```js
public async Task GetRankTableListTest()
{
    ...
    {
        var rankTableListJson = reqResult.GetRows();
        ...
        for (int i = 0; i < rankTableListJson.Count; i++)
        {
            ...
            var jObject = rankTableListJson[i] as JObject;
            if (jObject.ContainsKey("rankStartDateAndTime"))
            {
            ...
            if (jObject.ContainsKey("extraDataColumn"))
            {

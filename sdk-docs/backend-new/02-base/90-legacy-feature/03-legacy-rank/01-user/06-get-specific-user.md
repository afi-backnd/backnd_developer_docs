...
description: "특정 유저 랭킹 조회"
---

# GetUserRank
public Task&lt;RequestResult&gt; **GetUserRankAsync**(string **rankUuid**, string **userInDate**);  
public Task&lt;RequestResult&gt; **GetUserRankAsync**(string **rankUuid**, string **userInDate**, int **gap**);
...
```js
// 해당 유저의 랭킹만 조회
var reqResult = await BackndLegacy.URank.User.GetUserRankAsync("rankUuid", "userIndate");

// 위아래 3명의 유저를 포함하여 랭킹 조회
// 해당 유저가 4등인 경우 1 ~ 7등 유저가 조회됨
var reqResult2 = await BackndLegacy.URank.User.GetUserRankAsync("rankUuid", "userIndate", 3);
```

### Callback 방식
```js
// 해당 유저의 랭킹만 조회
BackndLegacy.URank.User.GetUserRank("rankUuid", "userIndate", callback =>
{
    // 이후 처리
});
...
BackndLegacy.URank.User.GetUserRank("rankUuid", "userIndate", 5, callback =>
{
    // 이후 처리
});
```
...
```js
public async Task GetUserRankTest()
{
    ...
    var reqResult = await BackndLegacy.URank.User.GetUserRankAsync(userUuid, userIndate, 3);

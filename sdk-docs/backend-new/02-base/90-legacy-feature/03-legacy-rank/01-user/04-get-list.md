...
description: "유저 랭킹 리스트 조회"
---

# GetRankList
public Task&lt;RequestResult&gt; **GetRankListAsync**(string **rankUuid**);  
public Task&lt;RequestResult&gt; **GetRankListAsync**(string **rankUuid**, int **limit**);  
public Task&lt;RequestResult&gt; **GetRankListAsync**(string **rankUuid**, int **limit**, int **offset**);
...
var reqResult = await BackndLegacy.URank.User.GetRankListAsync("rankUuid");
// rankUuid 랭킹에서 1 ~ 50등 랭커 조회
var reqResult2 = await BackndLegacy.URank.User.GetRankListAsync("rankUuid", 50);
// rankUuid 랭킹에서 11등 ~ 20등 랭커 조회
var reqResult3 = await BackndLegacy.URank.User.GetRankListAsync("rankUuid", 10, 10);
```

### Callback 방식
```js
// rankUuid 랭킹에서 1 ~ 10등 랭커 조회
BackndLegacy.URank.User.GetRankList("rankUuid", callback =>
{
    // 이후 처리
});
// rankUuid 랭킹에서 1 ~ 50등 랭커 조회
BackndLegacy.URank.User.GetRankList("rankUuid", 50, callback =>
{
    // 이후 처리
});
// rankUuid 랭킹에서 11등 ~ 20등 랭커 조회
BackndLegacy.URank.User.GetRankList("rankUuid", 10, 10, callback =>
{
    // 이후 처리
});
```
...
```js
public async Task GetMyRankTest()
{
    ...
    var reqResult = await BackndLegacy.URank.User.GetRankListAsync(userUuid, limit);
    if (reqResult.IsSuccess())
    {
        ...
        for (int i = 0; i < rankRows.Count; i++)
        {
            ...
            rankItem.gamerInDate = rankRows[i]["gamerInDate"].ToString();
            rankItem.nickname = rankRows[i]["nickname"].ToString();
            ...
            var jobj = rankRows[i] as JObject;
            if (jobj.ContainsKey(rankItem.extraName))
            {
                rankItem.extraData = rankRows[i][rankItem.extraName].ToString();
            }

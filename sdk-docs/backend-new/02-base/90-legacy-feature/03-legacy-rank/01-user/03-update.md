...
description: "유저 랭킹 갱신"
---

# UpdateUserScore
public Task&lt;RequestResult&gt; **UpdateUserScoreAsync**(string **rankUuid**, string **tableName**, string **rowIndate**, Param **param**);
...
* UpdateUserScore 함수는 [BackndLegacy.GameData.Update](/sdk-docs/backend/base/game-information/update/using-indate) 함수에 랭킹 갱신 기능이 추가된 함수 입니다. 
...
var reqResult = await BackndLegacy.URank.User.UpdateUserScoreAsync("랭킹 uuid", "테이블 이름", "갱신할 row inDate", param);
```
...
BackndLegacy.URank.User.UpdateUserScore("랭킹 uuid", "테이블 이름", "갱신할 row inDate", param, callback => 
{
    // 이후 처리
});
```
...
## Sample Code
```js
public async Task GetUpdateUserRankTest()
{
    ...
    var reqResult = await BackndLegacy.GameData.GetAsync("score", new Where());
    if (reqResult.IsSuccess())
    {
        if (reqResult.GetRows().Count > 0)
        {
            rowIndate = reqResult.GetRows()[0]["inDate"].ToString();
        }
        else
        {
            var reqResult2 = await BackndLegacy.GameData.InsertAsync(tableName, param);
            if (reqResult2.IsSuccess())
            {
                rowIndate = reqResult2.GetInDate();
            }
            else
            {
                return;
    ...
    var rankResult = await BackndLegacy.URank.User.UpdateUserScoreAsync(rankingUUID, tableName, rowIndate, param);

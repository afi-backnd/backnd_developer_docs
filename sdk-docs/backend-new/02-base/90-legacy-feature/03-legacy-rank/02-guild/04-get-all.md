...
description: "내 길드 랭킹 조회"
---

# GetMyGuildRank
public Task&lt;RequestResult&gt; **GetMyGuildRankAsync**(string **rankUuid**);  
public Task&lt;RequestResult&gt; **GetMyGuildRankAsync**(string **rankUuid**, int **gap**);
...
```js
// 자신이 가입한 길드의 랭킹만 조회
var reqResult = await BackndLegacy.URank.Guild.GetMyGuildRankAsync("rankUuid");

// 위아래 3개의 길드를 포함하여 랭킹 조회
// 자신의 길드가 4등인 경우 1 ~ 7등 길드가 조회됨
var reqResult2 = await BackndLegacy.URank.Guild.GetMyGuildRankAsync("rankUuid", 3);
```

### Callback 방식
```js
// 자신이 가입한 길드의 랭킹만 조회
BackndLegacy.URank.Guild.GetMyGuildRank("rankUuid", callback =>
{
    // 이후 처리
});
...
BackndLegacy.URank.Guild.GetMyGuildRank("rankUuid", 5 =>
{
    // 이후 처리
});
```
...
public async Task GetMyRankTest()
{
    ...
    var reqResult = await BackndLegacy.URank.Guild.GetMyGuildRankAsync(uuid, limit);

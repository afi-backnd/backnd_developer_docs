...
var reqResult = await BackndLegacy.URank.Guild.GetRankListAsync("rankUuid");
// rankUuid 랭킹에서 1 ~ 100등 길드 조회
var reqResult2 = await BackndLegacy.URank.Guild.GetRankListAsync("rankUuid", 100);
// rankUuid 랭킹에서 11등 ~ 20등 길드 조회
var reqResult3 = await BackndLegacy.URank.Guild.GetRankListAsync("rankUuid", 10, 10);
```

### Callback 방식
```js
// rankUuid 랭킹에서 1 ~ 10등 길드 조회
BackndLegacy.URank.Guild.GetRankList("rankUuid", callback=>
{
    // 이후 처리
});
// rankUuid 랭킹에서 1 ~ 100등 길드 조회
BackndLegacy.URank.Guild.GetRankList("rankUuid", 100, callback=>
{
    // 이후 처리
});
// rankUuid 랭킹에서 11등 ~ 20등 길드 조회
BackndLegacy.URank.Guild.GetRankList("rankUuid", 10, 10, callback=>
{
    // 이후 처리
});
```
...
```js
public async Task GetRankListTest()
{
    int limit = 43;

    List<GuildRankItem> rankItemList = new List<GuildRankItem>();

    var reqResult = await BackndLegacy.URank.Guild.GetRankListAsync(uuid, limit);

...
description: "데이터 수정 (inDate이용)"
---

# UpdateV2
public Task&lt;RequestResult&gt; **UpdateV2Async**(string **tableName**, string **inDate**, string **owner_inDate**, Param **param**);
...
## 설명
테이블에 저장되어 있는 값 중 inDate 컬럼의 값과 소유하는 유저의 owner_inDate가 일치하는 **row를 검색하여 수정**합니다.  
...
> 자신의 owner_inDate는 { BackndBase.UserInDate }를 통해 가져올 수 있습니다.  
...
// 자신의 inDate의 row를 수정
var reqResult = await BackndLegacy.GameData.UpdateV2Async("tableName", inDate, BackndBase.UserInDate, param);

// 타인의 inDate의 row를 수정
var reqResult2 = await BackndLegacy.GameData.UpdateV2Async("tableName", inDate, "owner_inDate", param);
```
...
// 자신의 inDate의 row를 수정
BackndLegacy.GameData.UpdateV2("tableName", inDate, BackndBase.UserInDate, param, (callback) =>
{
    // 이후 처리
});

// 타인의 inDate의 row를 수정
BackndLegacy.GameData.UpdateV2("tableName", inDate, "owner_inDate", param, (callback) =>
{
    // 이후 처리
});
```

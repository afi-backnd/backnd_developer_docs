...
public Task&lt;RequestResult&gt;  **GetMyDataAsync**(string **tableName**, Where **where**);  
public Task&lt;RequestResult&gt;  **GetMyDataAsync**(string **tableName**, Where **where**, int **limit**);  
public Task&lt;RequestResult&gt;  **GetMyDataAsync**(string **tableName**, Where **where**, int **limit**, string **firstKey**);  
public Task&lt;RequestResult&gt;  **GetMyDataAsync**(string **tableName**, Where **where**, string[] **select**, int **limit**);  
public Task&lt;RequestResult&gt;  **GetMyDataAsync**(string **tableName**, Where **where**,  string[] **select**, int **limit**, string **firstKey**);  
...
### Task 형식
```js
var reqResult = await BackndLegacy.GameData.GetMyDataAsync("tableName", new Where(), 10);
...
for (int i = 0; i < reqResult.GetRows().Count; ++i)
{
...
### Callback 형식
```js
BackndLegacy.GameData.GetMyData("tableName", new Where(), 10, reqResult =>
{ 
    if (reqResult.IsSuccess() == false)
    {
        // 요청 실패 처리
...
var reqResult = await BackndLegacy.GameData.GetMyDataAsync("tableName", where, 10);
if (reqResult.IsSuccess() == false)
{
    // 요청 실패 처리
    Debug.Log(reqResult);
...
//데이터 추출 방법 : 검색한 데이터 중 첫번째 데이터의 name 컬럼 출력
string name = reqResult.GetRows()[0]["name"].ToString();
...
BackndLegacy.GameData.GetMyData("tableName", where, 10, reqResult =>
{ 
    if (reqResult.IsSuccess() == false)
    {
        // 요청 실패 처리
...
{
    var reqResult = await BackndLegacy.GameData.GetMyDataAsync("PlayerInfo", new Where());
    ...
    GameDataItem gameDataItem = new GameDataItem(gameDataListJson[0] as JObject);

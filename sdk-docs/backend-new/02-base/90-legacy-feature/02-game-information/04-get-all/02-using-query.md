...
description: "쿼리로 전체 데이터 조회"
---

# Get

public Task&lt;RequestResult&gt; **GetAsync**(string **tableName**, Where **where**);  
public Task&lt;RequestResult&gt; **GetAsync**(string **tableName**, Where **where**, int **limit**);  
public Task&lt;RequestResult&gt; **GetAsync**(string **tableName**, Where **where**, int **limit**, string **firstKey**);  
public Task&lt;RequestResult&gt; **GetAsync**(string **tableName**, Where **where**, int **limit**, string **firstKey**, TableSortOrder **tableSortOrder**);  
public Task&lt;RequestResult&gt; **GetAsync**(string **tableName**, Where **where**, string[] **select**);  
public Task&lt;RequestResult&gt; **GetAsync**(string **tableName**, Where **where**, string[] **select**, int **limit**);  
public Task&lt;RequestResult&gt; **GetAsync**(string **tableName**, Where **where**, string[] **select**, int **limit**, string **firstKey**);  
public Task&lt;RequestResult&gt; **GetAsync**(string **tableName**, Where **where**, string[] **select**, int **limit**, string **firstKey**, TableSortOrder **tableSortOrder**);  
...
```js
// 조건 없이 모든 데이터 조회하기
var reqResult = await BackndLegacy.GameData.GetAsync("tableName", new Where(), 10);
...
{
    var inDate = rows[i]["inDate"].ToString();
    Debug.Log(inDate);
}
```
...
```js
// 조건 없이 모든 데이터 조회하기
BackndLegacy.GameData.Get("tableName", new Where(), 10, reqResult =>
{
    if (reqResult.IsSuccess() == false)
    {
        // 요청 실패 처리
...
var reqResult = await BackndLegacy.GameData.GetAsync("tableName", where, select, 10);
if (reqResult.IsSuccess() == false)
{
    // 요청 실패 처리
    Debug.Log(reqResult);
...
int score = (int)reqResult.GetRows()[0]["score"];
```
...
// 특정 유저의 데이터 중 deck과 score 컬럼만 조회
BackndLegacy.GameData.Get("tableName", where, select, 10, reqResult =>
{ 
    if (reqResult.IsSuccess() == false)
    {
        // 요청 실패 처리
...
{
    var reqResult = await BackndLegacy.GameData.GetAsync("PlayerInfo", new Where());
    ...
    var gameDataListJson = reqResult.GetRows();
    ...
    var gameDataItem = new GameDataItem(gameDataListJson[0] as JObject);

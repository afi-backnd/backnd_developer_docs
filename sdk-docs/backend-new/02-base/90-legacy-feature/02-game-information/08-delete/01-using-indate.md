...
description: "데이터 삭제(inDate이용)"
---

# DeleteV2
public Task&lt;RequestResult&gt; **DeleteV2Async**(string **tableName**, string **inDate**, string **owner_inDate**);
...
```js
// 자신의 inDate의 row를 제거
var reqResult = await BackndLegacy.GameData.DeleteV2Async("tableName", inDate, BackndBase.UserInDate);

//타인의 inDate의 row를 제거
var reqResult2 = await BackndLegacy.GameData.DeleteV2Async("tableName", inDate, "owner_inDate");
```

### Callback 형식
```js
// 자신의 inDate의 row를 제거
BackndLegacy.GameData.DeleteV2("tableName", inDate, BackndBase.UserInDate, (callback) =>
{
    // 이후 처리
});

//타인 inDate의 row를 제거
BackndLegacy.GameData.DeleteV2("tableName", inDate, "owner_inDate", (callback) =>
{
    // 이후 처리
});
```

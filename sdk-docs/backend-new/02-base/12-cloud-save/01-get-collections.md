...
description: "컬렉션 리스트 가져오기"
---

# GetCollectionNames
public Task&lt;GetCollectionNamesResult&gt; **GetCollectionNamesAsync**();    
...
### Task 형식
```js
var reqResult = await BackndCloudSave.Instance.GetCollectionNamesAsync();
if (reqResult.IsSuccess())
{
    foreach (var name in reqResult.GetNameList())
    {
        Debug.Log(name);
    }
}
...
### Callback 형식
```js
BackndCloudSave.Instance.GetCollectionNames(reqResult =>
{
    if (reqResult.IsSuccess())
    {
        foreach (var name in reqResult.GetNameList())

...
description: "유저 데이터 등록"
---

# CreateData

public Task&lt;CreateDataResult&gt; **CreateDataAsync**(string **tableName**);  
public Task&lt;CreateDataResult&gt; **CreateDataAsync**(string **tableName**, Param **param**);
...
- 해당 row는 CreateData 한 유저에게 귀속됩니다.  
...
### Task 형식
```js
var reqResult = await BackndUserData.Instance.CreateDataAsync("tableName", param);
```

### Callback 형식
```js
BackndUserData.Instance.CreateData("tableName", param, (callback) =>
{
  // 이후 처리
});
```
...
{
    var PlayerData = new PlayerDataItem();
    PlayerData.hp = 1000;
    PlayerData.mp = 0;
    ...
    var param = PlayerData.ToParam();
    var reqResult = await BackndUserData.Instance.CreateDataAsync("PlayerInfo", param);
    if (reqResult.IsSuccess())
    {
        string playerInfoIndate = reqResult.GetInDate();
        Debug.Log("내 playerInfo의 indate : " + playerInfoIndate);
    }
    else
    {
        Debug.LogError("게임 정보 등록 실패 : " + reqResult.ToString());
    }
}
```

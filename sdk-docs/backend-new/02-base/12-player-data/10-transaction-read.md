...
# ReadTransaction
public ReadTransactionResult **ReadTransactionAsync**(TransactionReadBuilder **builder**)

## 파라미터

| Value        | Type           | Description  | Default |
| :------------ |:-------------| :----- | :--- |
| builder   | TransactionReadBuilder | 읽기 전용 트랜잭션 명령 모음 클래스 | - |

### TransactionReadBuilder
데이터 불러오기 명령들을 추가하는 클래스입니다.

**전체 데이터중 최신 데이터 하나 불러오기**  
public TransactionReadBuilder **AddGetAllLatestData**(string **tableName**);  

```js
var readBuilder = new TransactionReadBuilder();
// 해당 테이블에서 가장 최근에 삽입된 데이터
readBuilder.AddGetAllLatestData("SCORE");
```

**자기 자신의 데이터 불러오기**  
public TransactionReadBuilder **AddGetLatestData**(string **tableName**);  
public TransactionReadBuilder **AddGetData**(string **tableName**, string **inDate**);  

```js
var readBuilder = new TransactionReadBuilder();

// 해당 테이블에서 가장 최근에 자기 자신이 등록한 데이터
readBuilder.AddGetLatestData("SCORE");
// 내가 등록한 데이터 중 inDate가 "2023-09-14T12:43:54.451Z"인 데이터 불러오기 
readBuilder.AddGetData("USER_DATA", "2023-09-14T12:43:54.451Z");
```

**타인의 데이터 불러오기**  
public TransactionReadBuilder **AddGetOtherLatestData**(string **tableName**, string **owner_inDate**);  
public TransactionReadBuilder **AddGetOtherData**(string **tableName**, string **inDate**, string **owner_inDate**);  
...
readBuilder.AddGetOtherLatestData("SCORE", otherOwnerIndate);
// 해당 테이블에서 owner_inDate가 일치하는 유저가 등록한 데이터 중 inDate가 "2023-09-14T12:43:54.451Z"인 데이터 불러오기
readBuilder.AddGetOtherData("USER_DATA", "2023-09-14T12:43:54.451Z", otherOwnerIndate);
```
...
* TransactionReadBuilder 내 명령 중 동일한 row을 불러올 경우, 에러가 발생합니다.  
...
readBuilder.AddGetAllLatestData("PUBLIC_DATA");
readBuilder.AddGetLatestData("ACHIVE_DATA");
readBuilder.AddGetData("USER_DATA", "2023-09-17T14:16:32.341Z");

var userResult = await BackndAuth.Instance.GetUserInfoByNicknameAsync("nickName");
var otherOwnerIndate = userResult.GetJObject()["row"]["inDate"].ToString();        

readBuilder.AddGetOtherLatestData("SCORE", otherOwnerIndate);        
readBuilder.AddGetOtherData("USER_DATA", "2023-09-19T16:21:12.542Z", otherOwnerIndate);

var reqResult = await BackndUserData.Instance.ReadTransactionAsync(readBuilder);
```
...
BackndUserData.Instance.ReadTransaction(readBuilder, callback =>
{
...
```js
public async Task TransactionReadBuilderTest()
{
    var readBuilder = new TransactionReadBuilder();
    readBuilder.AddGetAllLatestData("USER_DATA");
    readBuilder.AddGetData("USER_DATA", "2023-09-17T14:16:32.341Z");
    ...
    {
        var gameDataItem = new PlayerDataItem(gameDataListJson[i] as JObject);

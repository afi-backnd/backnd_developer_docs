...
{
    Debug.Log("쓰기량 : " + reqResult.GetJObject()["ConsumedCapacity"]["Write"]["CapacityUnits"]);
}
```
...
if (reqResult.IsSuccess())
{
    float capacity = reqResult.GetWriteCapacity();
...
if (reqResult.IsSuccess())
{
    float capacity = reqResult.GetReadCapacity();

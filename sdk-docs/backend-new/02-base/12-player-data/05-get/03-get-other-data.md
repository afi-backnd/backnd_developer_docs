...
description: "타인의 데이터 불러오기"
---

# GetOtherData
public Task&lt;GetDataResult&gt; **GetOtherDataAsync**(string **tableName**, string **inDate**, string **owner_inDate**);  
public Task&lt;GetDataResult&gt; **GetOtherDataAsync**(string **tableName**, string **inDate**, string **owner_inDate**, string[] **select**);  
public Task&lt;GetDataResult&gt; **GetOtherDataAsync**(string **tableName**);  
public Task&lt;GetDataResult&gt; **GetOtherDataAsync**(string **tableName**, int **limit**);  
public Task&lt;GetDataResult&gt; **GetOtherDataAsync**(string **tableName**, int **limit**, string **offsetKey**);  
public Task&lt;GetDataResult&gt; **GetOtherDataAsync**(string **tableName**, int **limit**, string **offsetKey**, TableSortOrder **sortOrder**);  
public Task&lt;GetDataResult&gt; **GetOtherDataAsync**(string **tableName**, string[] **select**);  
public Task&lt;GetDataResult&gt; **GetOtherDataAsync**(string **tableName**, string[] **select**, int **limit**);  
public Task&lt;GetDataResult&gt; **GetOtherDataAsync**(string **tableName**, string[] **select**, int **limit**, string **offsetKey**);  
public Task&lt;GetDataResult&gt; **GetOtherDataAsync**(string **tableName**, string[] **select**, int **limit**, string **offsetKey**, TableSortOrder **sortOrder**);  
...
* owner_inDate에 타인의 owner_inDate(BackndBase.UserInDate)를 입력할 경우, 타인의 데이터를 불러옵니다.
...
var userResult = await BackndAuth.Instance.GetUserInfoByNicknameAsync("nickName");
var otherOwnerIndate = userResult.GetJObject()["row"]["inDate"].ToString();

GetDataResult reqResult = null;
// tableName에서 최대 10개의 타인이 등록한 row 불러오기
reqResult = await BackndUserData.Instance.GetOtherDataAsync(tableName, otherOwnerIndate);
// 불러오기에 실패할 경우
...
reqResult = await BackndUserData.Instance.GetOtherDataAsync(tableName, otherOwnerIndate, 1);
...
reqResult = await BackndUserData.Instance.GetOtherDataAsync(tableName, otherOwnerIndate, 1, reqResult.GetOffsetKey());

reqResult = await BackndUserData.Instance.GetOtherDataAsync(tableName, otherOwnerIndate, 1, reqResult.GetOffsetKey(), TableSortOrder.DESC);
...
reqResult = await BackndUserData.Instance.GetOtherDataAsync(tableName, otherOwnerIndate, selectList);

// tableName에서 최대 1 타인이 등록한 row 불러오기(selectList에 표시된 컬럼 외에는 표시되지 않습니다.)
reqResult = await BackndUserData.Instance.GetOtherDataAsync(tableName, otherOwnerIndate, selectList, 1);

// 위에서 호출한 최대 1개의 타인이 등록한 row 불러오기 이후의 데이터가 존재할 경우, 그 이후의 데이터 불러오기
// (최신순 -> 오래된 순으로 데이터가 불러와집니다.)
// (selectList에 표시된 컬럼 외에는 표시되지 않습니다.)
reqResult = await BackndUserData.Instance.GetOtherDataAsync(tableName, otherOwnerIndate, selectList, 1, reqResult.GetOffsetKey());

reqResult = await BackndUserData.Instance.GetOtherDataAsync(tableName, otherOwnerIndate, selectList, 1, reqResult.GetOffsetKey(), TableSortOrder.DESC);

// 위에서 호출한 불러오기의 값 중, inDate가 일치하는 데이터를 불러오기
reqResult = await BackndUserData.Instance.GetOtherDataAsync(tableName, reqResult.GetRows()[0]["owner_inDate"].ToString(), reqResult.GetRows()[0]["inDate"].ToString());

// 위에서 호출한 불러오기의 값 중, inDate가 일치하는 데이터를 불러오기(selectList에 표시된 컬럼 외에는 표시되지 않습니다.)
reqResult = await BackndUserData.Instance.GetOtherDataAsync(tableName, reqResult.GetRows()[0]["owner_inDate"].ToString(), reqResult.GetRows()[0]["inDate"].ToString(), selectList);
```
...
// 유저 이름을 검색하여 해당 유저의 owner_inDate 알아내기
var userResult = await BackndAuth.Instance.GetUserInfoByNicknameAsync("nickName");
string otherOwnerIndate = userResult.GetJObject()["row"]["inDate"].ToString();

string[] selectList = new string[] { "intData", "doubleData" };
BackndUserData.Instance.GetOtherData(tableName, otherOwnerIndate, callback =>
{
...
BackndUserData.Instance.GetOtherData(tableName, otherOwnerIndate, 1, callback =>
{
    if (callback.IsSuccess() && callback.GetRows().Count <= 0)
    {
        Debug.Log("데이터가 존재하지 않습니다");
    ...
    BackndUserData.Instance.GetOtherData(tableName, otherOwnerIndate, 1, callback.GetOffsetKey(), callback2 =>
    {
        // callback2 처리
    });
    BackndUserData.Instance.GetOtherData(tableName, otherOwnerIndate, 1, callback.GetOffsetKey(), TableSortOrder.DESC, callback2 =>
    {
        // callback2 처리
    });
});
...
BackndUserData.Instance.GetOtherData(tableName, otherOwnerIndate, selectList, callback =>
{
    if (callback.IsSuccess() && callback.GetRows().Count <= 0)
    {
        Debug.Log("데이터가 존재하지 않습니다");
...
BackndUserData.Instance.GetOtherData(tableName, otherOwnerIndate, selectList, 1, callback =>
{
    if (callback.IsSuccess() && callback.GetRows().Count <= 0)
    {
        Debug.Log("데이터가 존재하지 않습니다");
    ...
    BackndUserData.Instance.GetOtherData(tableName, otherOwnerIndate, selectList, 1, callback.GetOffsetKey(), callback2 =>
    {
        // callback2 처리
    });
    BackndUserData.Instance.GetOtherData(tableName, otherOwnerIndate, selectList, 1, callback.GetOffsetKey(), TableSortOrder.DESC, callback2 =>
    {
        // callback2 처리
    });
});
...
// 위에서 호출한 불러오기의 값 중, inDate가 일치하는 데이터를 불러오기
BackndUserData.Instance.GetOtherData(tableName, reqResult.GetRows()[0]["owner_inDate"].ToString(), reqResult.GetRows()[0]["inDate"].ToString(), callback =>
{

});
// 위에서 호출한 불러오기의 값 중, inDate가 일치하는 데이터를 불러오기(selectList에 표시된 컬럼 외에는 표시되지 않습니다.)
BackndUserData.Instance.GetOtherData(tableName, reqResult.GetRows()[0]["owner_inDate"].ToString(), reqResult.GetRows()[0]["inDate"].ToString(), selectList, callback =>
{
...
{
    var userResult = await BackndAuth.Instance.GetUserInfoByNicknameAsync("nickName");
    var gamerIndate = userResult.GetJObject()["row"]["inDate"].ToString();

    var reqRequest = await BackndUserData.Instance.GetOtherDataAsync("PlayerInfo", gamerIndate);

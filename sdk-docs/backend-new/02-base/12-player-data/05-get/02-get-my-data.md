---
sidebar_label: 내 데이터만 불러오기
description: "내 데이터만 불러오기"
---

# GetDataAsync
public Task&lt;GetDataResult&gt; **GetDataAsync**(string **tableName**);  
public Task&lt;GetDataResult&gt; **GetDataAsync**(string **tableName**, int **limit**);  
public Task&lt;GetDataResult&gt; **GetDataAsync**(string **tableName**, int **limit**, string **offsetKey**);  
public Task&lt;GetDataResult&gt; **GetDataAsync**(string **tableName**, int **limit**, string **offsetKey**, TableSortOrder **sortOrder**);  
public Task&lt;GetDataResult&gt; **GetDataAsync**(string **tableName**, string[] **select**);  
public Task&lt;GetDataResult&gt; **GetDataAsync**(string **tableName**, string[] **select**, int **limit**);  
public Task&lt;GetDataResult&gt; **GetDataAsync**(string **tableName**, string[] **select**, int **limit**, string **offsetKey**);  
public Task&lt;GetDataResult&gt; **GetDataAsync**(string **tableName**, string[] **select**, int **limit**, string **offsetKey**, TableSortOrder **sortOrder**);  
public Task&lt;GetDataResult&gt; **GetDataAsync**(string **tableName**, GetUserDataParams **getParams**);  
public Task&lt;GetDataResult&gt; **GetDataAsync**(string **tableName**, string **inDate**);  
public Task&lt;GetDataResult&gt; **GetDataAsync**(string **tableName**, string **inDate**, string[] **select**);  
...
* private, public 테이블 모두 자기 자신의 데이터만 조회할 수 있습니다.  
...
reqResult = await BackndUserData.Instance.GetDataAsync(tableName);
// 불러오기에 실패할 경우
...
reqResult = await BackndUserData.Instance.GetDataAsync(tableName, 1);
//  위에서 호출한 최대 1개의 자기 자신이 등록한 row 불러오기 이후, 그 아래로 더 불러올 오래된 데이터가 있는지 확인
if (reqResult.HasOffsetKey() == false)
{
    Debug.Log("불러올 데이터가 존재하지 않습니다");
}

// 위에서 호출한 최대 1개의 자기 자신이 등록한 row 불러오기 이후의 데이터가 존재할 경우, 그 이후의 데이터 불러오기
// (최신순 -> 오래된 순으로 데이터가 불러와집니다.)
reqResult = await BackndUserData.Instance.GetDataAsync(tableName, 1, reqResult.GetOffsetKey());

// 데이터를 불러올 때 selectList에 포함된 컬럼만 보여줍니다.
string[] selectList = new string[] { "intData", "doubleData" };

// tableName에서 최대 10개의 자기 자신이 등록한 row 불러오기(selectList에 표시된 컬럼 외에는 표시되지 않습니다.)
reqResult = await BackndUserData.Instance.GetDataAsync(tableName, selectList);

// tableName에서 최대 1 자기 자신이 등록한 row 불러오기(selectList에 표시된 컬럼 외에는 표시되지 않습니다.)
reqResult = await BackndUserData.Instance.GetDataAsync(tableName, selectList, 1);

// 위에서 호출한 최대 1개의 자기 자신이 등록한 row 불러오기 이후의 데이터가 존재할 경우, 그 이후의 데이터 불러오기
// (최신순 -> 오래된 순으로 데이터가 불러와집니다.)
// (selectList에 표시된 컬럼 외에는 표시되지 않습니다.)
reqResult = await BackndUserData.Instance.GetDataAsync(tableName, selectList, 1, reqResult.GetOffsetKey());

// 위에서 호출한 불러오기의 값 중, inDate가 일치하는 데이터를 불러오기
reqResult = await BackndUserData.Instance.GetDataAsync(tableName, reqResult.GetRows()[0]["inDate"].ToString());

// 위에서 호출한 불러오기의 값 중, inDate가 일치하는 데이터를 불러오기(selectList에 표시된 컬럼 외에는 표시되지 않습니다.)
reqResult = await BackndUserData.Instance.GetDataAsync(tableName, reqResult.GetRows()[0]["inDate"].ToString(), selectList);
```
...
BackndUserData.Instance.GetData(tableName, callback =>
{
...
BackndUserData.Instance.GetData(tableName, 1, callback =>
{
    if (callback.IsSuccess() && callback.GetRows().Count <= 0)
    {
        Debug.Log("데이터가 존재하지 않습니다");
    ...
    BackndUserData.Instance.GetData(tableName, 1, callback.GetOffsetKey(), callback2 =>
    {
        // callback2 처리
    });
});

BackndUserData.Instance.GetData(tableName, selectList, callback =>
{
    if (callback.IsSuccess() && callback.GetRows().Count <= 0)
    {
        Debug.Log("데이터가 존재하지 않습니다");
...
BackndUserData.Instance.GetData(tableName, selectList, 1, callback =>
{
    if (callback.IsSuccess() && callback.GetRows().Count <= 0)
    {
        Debug.Log("데이터가 존재하지 않습니다");
    ...
    BackndUserData.Instance.GetData(tableName, selectList, 1, callback.GetOffsetKey(), callback2 =>
    {
        // callback2 처리
    });
});
...
// 위에서 호출한 불러오기의 값 중, inDate가 일치하는 데이터를 불러오기
BackndUserData.Instance.GetData(tableName, reqRequest.GetRows()[0]["inDate"].ToString(), callback =>
{

});
// 위에서 호출한 불러오기의 값 중, inDate가 일치하는 데이터를 불러오기(selectList에 표시된 컬럼 외에는 표시되지 않습니다.)
BackndUserData.Instance.GetData(tableName, reqRequest.GetRows()[0]["inDate"].ToString(), selectList, callback =>
{
...
### Error cases
**inDate를 이용한 GetMyData 시, inDate가 string.Empty일 경우**  
statusCode : 400  
...
{
    var reqRequest = await BackndUserData.Instance.GetDataAsync("PlayerInfo");

...
### Task 방식
```js
var reqResult = await BackndLeaderboard.Guild.GetRankRewardsAsync("uuid");
...
### Callback 방식
```js
BackndLeaderboard.Guild.GetRankRewards("leaderboardUuid", reqResult =>
{
    foreach (var info in reqResult.GetInfoList())
    {
        Debug.Log(info);

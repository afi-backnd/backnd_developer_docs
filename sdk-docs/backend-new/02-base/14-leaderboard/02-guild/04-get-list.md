...
description: "리더보드 내 전체 순위 조회"
---

# GetRanks
public Task&lt;GuildRankResult&gt; **GetRanksAsync**(string **leaderboardUuid**);  
public Task&lt;GuildRankResult&gt; **GetRanksAsync**(string **leaderboardUuid**, int **limit**);  
public Task&lt;GuildRankResult&gt; **GetRanksAsync**(string **leaderboardUuid**, int **limit**, int **offset**);
...
reqResult = await BackndLeaderboard.Guild.GetRanksAsync("leaderboardUuid", 50);
// leaderboardUuid 랭킹에서 51 ~ 100등 랭커 조회
reqResult = await BackndLeaderboard.Guild.GetRanksAsync("leaderboardUuid", 50, 50);
// leaderboardUuid 랭킹에서 101 ~ 150등 랭커 조회
reqResult = await BackndLeaderboard.Guild.GetRanksAsync("leaderboardUuid", 50, 100);

// example 2. 특정 위치 조회 및 기본값 조회
// leaderboardUuid 랭킹에서 11등 ~ 30등 랭커 조회
reqResult = await BackndLeaderboard.Guild.GetRanksAsync("leaderboardUuid", 20, 10);
// leaderboardUuid 랭킹에서 1 ~ 10등 랭커 조회
reqResult = await BackndLeaderboard.Guild.GetRanksAsync("leaderboardUuid");
...
// example 1. 1위부터 150위까지 조회.
// leaderboardUuid 랭킹에서 1 ~ 50등 랭커 조회
BackndLeaderboard.Guild.GetRanks("leaderboardUuid", 50, callback =>
{
    ...
    Debug.Log("리더보드 총 길드 등록 수 : " + callback.GetTotalCount());
...
// leaderboardUuid 랭킹에서 51 ~ 100등 랭커 조회
BackndLeaderboard.Guild.GetRanks("leaderboardUuid", 50, 50, callback =>
{
    // 이후 처리
});
// leaderboardUuid 랭킹에서 101 ~ 150등 랭커 조회
BackndLeaderboard.Guild.GetRanks("leaderboardUuid", 50, 100, callback =>
{
    // 이후 처리
});
...
// example 2. 특정 위치 조회 및 기본값 조회
// leaderboardUuid 랭킹에서 11등 ~ 30등 랭커 조회
BackndLeaderboard.Guild.GetRanks("leaderboardUuid", 20, 10, callback =>
{
    // 이후 처리
});
// leaderboardUuid 랭킹에서 1 ~ 10등 랭커 조회
BackndLeaderboard.Guild.GetRanks("leaderboardUuid", callback =>
{
    // 이후 처리
});
```

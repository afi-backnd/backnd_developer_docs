...
description: "길드 리더보드 과거 전체 순위 확인"
---

# GetPastRanks
public Task&lt;GuildRankResult&gt; **GetPastRanksAsync**(string **leaderboardUuid**, int **limit = 10**, int **offset = 0**)

## 파라미터
| Value        | Type           | Description  | Default |  
| :------------ |:-------------| :----- | :----- |
| leaderboardUuid | string | 조회할 리더보드의 uuid | -  |
| limit | int | 불러올 리더보드 길드의 수 (1 ~ 50) | 10 |  
...
{    
    public List<GuildRankInfo> GetInfoList();
}
```
...
```js
// 리더보드 1위 부터 10위까지
var reqResult = await BackndLeaderboard.Guild.GetPastRanksAsync("01920445-a435-759e-bc3b-048a1e7a87aa");

foreach (var info in reqResult.GetInfoList())
{
    string name = $"[{info.Rank}] {info.GuildName} : {info.Score}";
    Debug.Log(info);
}

// 리더보드 11위(offset + 1) 부터 61위까지((offset + 1 + limit)
reqResult = await BackndLeaderboard.Guild.GetPastRanksAsync("a61c6980-db5f-11ee-95a9-73cbb1225013", 50, 10);
```

### Callback 방식
```js
BackndLeaderboard.Guild.GetPastRanks("a61c6980-db5f-11ee-95a9-73cbb1225013", 10, 0, callback =>
{

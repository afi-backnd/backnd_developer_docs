...
var reqResult = await BackndLeaderboard.User.UpdateLeaderboardWithDataAsync
    ("리더보드 uuid", "테이블 이름", "갱신할 row inDate", param);
```
...
var reqResult = await BackndLegacy.URank.User.UpdateUserScore("랭킹 uuid", "테이블 이름", "갱신할 row inDate", param);
```
...
### BackndLeaderboard.User.UpdateLeaderboardWithDataAsync
```js
{
    "rows": [
        {
...
### BackndLegacy.URank.User.UpdateUserScore

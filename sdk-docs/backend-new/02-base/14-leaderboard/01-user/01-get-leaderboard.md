...
### Task 형식
```js
var reqResult = await BackndLeaderboard.User.GetLeaderboardsAsync();
...
### Callback 형식
```js
BackndLeaderboard.User.GetLeaderboards((callback) =>
{
...
```js
{
    "rows": [
        {
            "rankType": "user", // 리더보드 타입(user / guild)
            ...
            "extraDataColumn": "추가 항목 컬럼 이름", // (추가항목 선택시에만) 추가항목 컬럼이름
            "extraDataType": "S", // (추가항목 선택시에만) 추가항목 컬럼 데이터 타입
            ...
            "rewardName": "보상 차트 이름",
            "title": "랭킹 제목",
            "table": "랭킹에 사용된 테이블 이름",
            "column": "랭킹에 사용된 컬럼 이름"
        },
        ...
    ]
}

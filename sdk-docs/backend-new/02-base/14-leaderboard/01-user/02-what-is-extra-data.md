...
var reqResult = await BackndLeaderboard.User.UpdateLeaderboardWithDataAsync
	("aa6609c0-****-****-****-ffd79d523912", "DAILY_USER_RANK", leaderboardDataIndate, leaderboardParam);
...
```js
var reqResult = await BackndLeaderboard.User.GetRanksAsync("aa6609c0-****-****-****-ffd79d523912");
foreach (var info in reqResult.GetInfoList())
{
    int level = int.Parse(info.ExtraData);
}
```
...
public void InsertRank() {
	...
	var reqResult = await BackndLeaderboard.User.UpdateLeaderboardWithDataAsync
		("aa6609c0****-ffd79d523912", "tableName", "rowInDate", param);
}
```
...
foreach (var info in reqResult.GetInfoList())
{
    string[] extraData = info.ExtraData.Split("|");

...
description: "길드원 리스트 조회"
---

# GetGuildMembers

public Task< GetGuildMembersResult > **GetGuildMembersAsync**(string **guildIndate**);  
public Task< GetGuildMembersResult > **GetGuildMembersAsync**(string **guildIndate**, int **limit**);  
public Task< GetGuildMembersResult > **GetGuildMembersAsync**(string **guildIndate**, string **firstKey**);  
public Task< GetGuildMembersResult > **GetGuildMembersAsync**(string **guildIndate**, int **limit**, string **firstKey**);
...
```js
var reqResult = await BackndGuild.Instance.GetGuildMembersAsync("guildIndate");
reqResult = await BackndGuild.Instance.GetGuildMembersAsync("guildIndate", 5);

// 처음 리스트를 읽을 때, firstkey 값을 저장해 놓은 뒤,
reqResult = await BackndGuild.Instance.GetGuildMembersAsync("guildIndate");
var firstKey = reqResult.GetOffsetKey();
// 다음 페이지 넘어갈 때 firstKey를 offset으로 하여 다시 읽어옴
reqResult = await BackndGuild.Instance.GetGuildMembersAsync("guildIndate", firstKey);
reqResult = await BackndGuild.Instance.GetGuildMembersAsync("guildIndate", 5, firstKey);
```

### Callback 방식

```js
BackndGuild.Instance.GetGuildMembers("guildIndate", (callback) =>
{
  // 이후 처리
});

BackndGuild.Instance.GetGuildMembers("guildIndate", 5, (callback) =>
{
  // 이후 처리
});

// 동기 함수와 같은 방식으로 firstKey를 저장한 후, 다음 리스트를 불러옴
BackndGuild.Instance.GetGuildMembers("guildIndate", firstKey, (callback) =>
{
  // 이후 처리
});

BackndGuild.Instance.GetGuildMembers("guildIndate", 20, firstKey, (callback) =>
{
  // 이후 처리
});
```
...
```js
public async Task GetGuildMemberListV3()
{
    var reqResult = await BackndGuild.Instance.GetGuildMembersAsync("2021-10-29T06:32:10.147Z");
    ...
    for (int i = 0; i < infoList.Count; i++)
    {
        ...
        member.nickname = info.UserNickname;
        member.inDate = info.GuildInDate;
        member.gamerInDate = info.UserInDate;
        member.lastLogin = info.LastSignIn;
        member.position = info.Position;

        var goodsList = info.TotalGoodsAmount;
        for(var k = 0; k < goodsList.Length; k++)
        {
            var number = k + 1;
            member.totalGoodsAmount.Add($"totalGoods{number}Amount", int.Parse(goodsList[k]));
        }
